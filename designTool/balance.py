...
def balance(airplane):
    ...
    xcg_hist, W_hist, xcg_fwd, xcg_aft, xcg_payload = cg_range(payload_groups, xcg_empty, W_empty, xcg_crew, W_crew, xcg_fuel, W_fuel, cm_w)
    ...
    airplane['balance']['xcg_payload'] = xcg_payload
    airplane['balance']['CLv'] = CLv
    airplane['balance']['xcg_hist'] = xcg_hist
    airplane['balance']['W_hist'] = W_hist
...
def cg_range(payload_groups, xcg_empty, W_empty, xcg_crew, W_crew, xcg_fuel, W_fuel, cm_w):
    '''
    This function computes the CG range for various weight conditions.
    The following loading procedure is implemented:
    empty -> +crew -> +fuel -> +payload -> -fuel -> -payload -> -crew
    '''

    # Convert payload information into lists containing the weight and position of each payload unit

    W_payload_units = []
    x_payload_units = []

    for group_id in range(len(payload_groups['weight'])):

        W_unit = payload_groups['weight'][group_id]
        x0 = payload_groups['x0'][group_id]
        pitch = payload_groups['pitch'][group_id]
        units = payload_groups['units'][group_id]

        W_payload_units = W_payload_units + units*[W_unit]
        x_payload_units = x_payload_units + list(x0 + pitch*np.arange(units))

    W_payload_units = np.array(W_payload_units)
    x_payload_units = np.array(x_payload_units)

    # Overall payload CG
    xcg_payload = sum(W_payload_units*x_payload_units)/sum(W_payload_units)

    W_hist = []
    xcg_hist = []
    
    # W_curr and xcg_curr will store the current weight and CG position as we load the aircraft

    # Empty airplane
    xcg_curr = xcg_empty
    W_curr = W_empty
    
    xcg_hist.append(xcg_curr)
    W_hist.append(W_curr)

    # Add crew
    xcg_curr = (xcg_curr*W_curr + xcg_crew*W_crew)/(W_curr + W_crew)
    W_curr = W_curr + W_crew

    W_hist.append(W_curr)
    xcg_hist.append(xcg_curr)

    # Add fuel
    xcg_curr = (xcg_curr*W_curr + xcg_fuel*W_fuel)/(W_curr + W_fuel)
    W_curr = W_curr + W_fuel

    W_hist.append(W_curr)
    xcg_hist.append(xcg_curr)

    # Add payload from front to back
    for WW,xx in zip(W_payload_units,x_payload_units):
        xcg_curr = (xcg_curr*W_curr + xx*WW)/(W_curr + WW)
        W_curr = W_curr + WW
    ...
    # Remove payload from front to back
    for WW,xx in zip(W_payload_units,x_payload_units):
        xcg_curr = (xcg_curr*W_curr - xx*WW)/(W_curr - WW)
        W_curr = W_curr - WW
    ...
    # Convert to arrays to facilitate further operations
    xcg_hist = np.array(xcg_hist)
    W_hist = np.array(W_hist)

    # Find CG range (adding a 2% margin on either side)
    xcg_fwd = min(xcg_hist) - 0.02*cm_w
    xcg_aft = max(xcg_hist) + 0.02*cm_w

    return xcg_hist, W_hist, xcg_fwd, xcg_aft, xcg_payload