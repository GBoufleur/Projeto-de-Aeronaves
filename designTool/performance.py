...
from .propulsion import engineTSFC, lowspeed_thrust_lapse
...
def performance(W0, W_cruise, airplane,
    ...
    # Thrust lapse due to altitude and temperature
    kT = lowspeed_thrust_lapse(altitude_takeoff, deltaISA_takeoff)
    ...
    T0W0 = kT*0.2387/sigma/CLmaxTO/distance_takeoff*W0/S_w
    ...
    if landing_method == 'Roskam':
        ...
        f_land = 5/3 # Landing distance safety factor (FAR Part 121)
    ...
    def climb_analysis(grad, Ks, altitude,
                       ...
                       max_continuous=False, deltaISA=0):
        ...
        # Thrust lapse due to altitude and temperature variations
        kT = lowspeed_thrust_lapse(altitude_takeoff, deltaISA_takeoff)

        # Thrust reduction for maximum continuous condition,
        # as recommended by Roskam.
        if max_continuous:
            kT = 0.94*kT
